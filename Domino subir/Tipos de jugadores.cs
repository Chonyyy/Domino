...
namespace Tipos_de_Jugadores
{
    ...
    /* public class Tramposo : JUGADOR
     {
         //Mascara booleana para recordar las fichas del jugador siguiente
         bool[] mask = new bool[Domino.NumeroMaximoFichas + 1];
         bool[] mask_extremos = new bool[2];
         int SiguienteJugador = MESA.jugadorActual + 1; 

         public override FICHA Escoger()
         {
             FICHA f = new FICHA(Fichas_posibles[0].num1, Fichas_posibles[0].num2); ;
             //FICHA f = new FICHA(FichasDelJugador[0].num1, FichasDelJugador[0].num2);
         ...
         private void MirarFichas()
         {
             for (int i = 0; i < Domino.jugadores[SiguienteJugador].FichasDelJugador.Count; i++)
             {
                 //Agrega a la mask los numeros q tiene el jugador siguiente
                 mask[Domino.jugadores[SiguienteJugador].FichasDelJugador[i].num1] = true;
                 mask[Domino.jugadores[SiguienteJugador].FichasDelJugador[i].num2] = true;

                 //agrega a mask_extremos si el prox jugador lleva alguna ficha para jugar
                 //compara el 1er extremo
                 if (MESA.Mesa.Count != 0 && (MESA.Extremos().num1 == Domino.jugadores[SiguienteJugador].FichasDelJugador[i].num1
                  || MESA.Extremos().num1 == Domino.jugadores[MESA.jugadorActual + 1].FichasDelJugador[i].num2))
                     mask_extremos[0] = true;

                 //compara el 2do extremo
                 if (MESA.Mesa.Count != 0 && (MESA.Extremos().num2 == Domino.jugadores[SiguienteJugador].FichasDelJugador[i].num1
                 || MESA.Extremos().num2 == Domino.jugadores[MESA.jugadorActual + 1].FichasDelJugador[i].num2))
                     mask_extremos[1] = true;
             }
         }
     }*/
    /* public class Inteligente : JUGADOR
     {
         //cuando tenga la ficha escogida siiiii pega en ambos extremos jugar por donde mas yo tenga o
         //por el lado que mi companero se paso 

         int[] mayorCantidad = new int[Domino.NumeroMaximoFichas + 1];
         public override FICHA Escoger()
         {
             FICHA ficha = new FICHA(FichasDelJugador[0].num1, FichasDelJugador[0].num2);

             //array para saber las fichas que mas tengo
             //cada posicion del array es el numero de huequitos y en numero en esa posicion la cantidad max que tiene
             //LLenarArray();

             //jugar por el extremo que mas fichas tenga       
             int n1 = mayorCantidad[MESA.Extremos().num1] > mayorCantidad[MESA.Extremos().num2] ?
                      mayorCantidad[MESA.Extremos().num1] : mayorCantidad[MESA.Extremos().num2];

             //coger una ficha que no lleve el jugador siguiente y que el actual tenga varias
             for (int i = 0; i < Domino.jugadores[MESA.jugadorActual + 1].Pases.Count; i++)
             {
                 FICHA comp = new FICHA(n1, i);

                 for (int j = 0; j < Domino.jugadores[MESA.jugadorActual].fichas_que_Lleva_Jugador(MESA.Extremos()).Count; j++)
                 {
                     //si encuentra la ficha con estas condiciones la elige
                     if (Domino.jugadores[MESA.jugadorActual].fichas_que_Lleva_Jugador(MESA.Extremos())[j] == comp)
                     {
                         ficha = comp;
                     }
                 }
             ...
             //------------------------------------------------------------------------------
             for (int p = 0; p < Domino.jugadores[MESA.jugadorActual+1].Pases.Count; p++)
             {
                 if (MESA.Extremos().num1 == Pases[p])
                 {

                 }
                 else if (MESA.Extremos().num1 == Pases[p])
                 { 
         ...
         {
             for (int i = 0; i < Domino.jugadores[MESA.jugadorActual].FichasDelJugador.Count; i++)
             {
                 mayorCantidad[Domino.jugadores[MESA.jugadorActual].FichasDelJugador[i].num1]++;
                 mayorCantidad[Domino.jugadores[MESA.jugadorActual].FichasDelJugador[i].num2]++;
             }
